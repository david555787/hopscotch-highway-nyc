...
window.createHighwayArt = function(g) {
  ...
  // Anchor the generated train at its wheel-contact line instead of the image's lower edge.
  const trackAngle=Math.atan(.25),trainWheelAnchor=.8;
  let sprites=[],loaded=0;
  const start=document.querySelector('#play');start.disabled=true;start.textContent='Loading artwork…';
  function artworkLoaded(){loaded++;if(loaded>=3){start.disabled=false;start.textContent=state.round===1?'Start round':'Play next round';}}
  ...
  atlas.onload=()=>{
    ...
    cells.forEach((cell,i)=>sprites[offset+i]=cell);artworkLoaded();
  };
  atlas.src=url;}
  function loadSprite(url,index){const image=new Image();image.onerror=()=>{start.textContent='Artwork could not load — refresh';};image.onload=()=>{sprites[index]={image,x:0,y:0,w:image.width,h:image.height};artworkLoaded();};image.src=url;}
  loadAtlas('world-atlas.png',0);loadAtlas('nyc-atlas.png',6);loadSprite('nyc-train-v2.png',12);
  ...
  function sprite(i,x,y,width,z=0,flip=false,angle=0,anchorY=1){const r=sprites[i];if(!r)return;const p=project(x,y,z),h=width*r.h/r.w;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(angle);if(flip)ctx.scale(-1,1);ctx.drawImage(r.image,r.x,r.y,r.w,r.h,-width/2,-h*anchorY,width,h);ctx.restore();}
